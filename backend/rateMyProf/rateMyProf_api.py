...
class RateMyProfApi:
    ...
    def scrape_professors(
        ...
        for i in range(1, num_of_pages + 1):  # the loop insert all professor into list
            ...
            for json_professor in json_response["professors"]:
                ...
                # Get department, default to empty string if not present
                department = json_professor.get("tDept", "")
                professor = Professor(
                    json_professor["tid"],
    ...
    def get_professors_by_department(
        ...
        '''
        Return a list of all professors in the specified department.
        Case insensitive matching.
        
        Args:
            department: The department name to filter by (e.g., "Computer Science", "CS")
        
        Returns:
            A list of Professor objects matching the department
        '''
        department = department.strip().lower()
        matching_professors = []
        
        for professor in self.professors.values():
            if professor.department and professor.department.lower() == department:
                matching_professors.append(professor)
        
        return matching_professors

    def get_professors_by_department_partial(
        self, department: str
    ) -> list[Professor]:
        '''
        Return a list of all professors whose department contains the search term.
        Case insensitive partial matching.
        
        Args:
            department: A partial department name to search for (e.g., "Computer", "Science")
        
        Returns:
            A list of Professor objects whose department contains the search term
        '''
        department = department.strip().lower()
        matching_professors = []
        
        for professor in self.professors.values():
            if professor.department and department in professor.department.lower():
                matching_professors.append(professor)
    ...
    def get_all_departments(self) -> list[str]:
        ...
        Returns:
            A sorted list of unique department names (empty strings excluded)
        '''
        departments = set()
        for professor in self.professors.values():
            if professor.department and professor.department.strip():
                departments.add(professor.department)
        
        return sorted(list(departments))

    def get_professors_by_departments(
        self, departments: list[str]
    ) -> dict[str, list[Professor]]:
        '''
        Return a dictionary mapping department names to lists of professors.
        Useful for getting professors from multiple departments at once.
        
        Args:
            departments: A list of department names to filter by
        
        Returns:
            A dictionary where keys are department names and values are lists of Professor objects
        '''
        result = {}
        for dept in departments:
            result[dept] = self.get_professors_by_department(dept)
...
if __name__ == '__main__':
    ...
    # University ID 1038 for your university
    api = RateMyProfApi(1038)
    
    # Example: Get all departments
    print("Available departments:")
    departments = api.get_all_departments()
    for dept in departments[:10]:  # Print first 10
        print(f"  - {dept}")
    
    # Example: Get professors by exact department match
    cs_professors = api.get_professors_by_department("Computer Science")
    print(f"\nComputer Science professors: {len(cs_professors)}")
    for prof in cs_professors[:5]:  # Print first 5
        print(f"  - {prof.name} (Rating: {prof.overall_rating})")
    
    # Example: Get professors by partial department match
    math_professors = api.get_professors_by_department_partial("Math")
    print(f"\nMath-related professors: {len(math_professors)}")
    
    # Example: Get professors from multiple departments
    dept_dict = api.get_professors_by_departments(["Computer Science", "Mathematics"])
    for dept, profs in dept_dict.items():
        print(f"\n{dept}: {len(profs)} professors")
